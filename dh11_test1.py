...
def temp():
    ...
    if check == tmp and temperature<50 and humidity<90 :
        print time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
        print('数据正确')
        temper = str(temperature)+"."+str(temperature_point)
        humidi = str(humidity)+"."+ str(humidity_point)
        print("温度: "+temper+", 湿度: "+humidi)
        outstr = u'数据正确',u"温度: "+str(temperature)+u", 湿度: "+str(humidity)
        SQL = "INSERT INTO temptest (error, wendu,shidu,times) VALUES ('0', '{wendu}','{shidu}','{shijian}')".format(wendu = temper,shidu = humidi,shijian = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
        mysql_use.execute_to_mysql(SQL)
    else:
        print time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()),
        print('数据错误')
        print("温度: "+str(temperature)+",湿度: "+str(humidity)+"校验值: "+str(check)+" 检查值: "+str(tmp))
        outstr = u'数据错误',u"温度: "+str(temperature)+u",湿度: "+str(humidity)+u"校验值: "+str(check)+u" 检查值: "+str(tmp)
    GPIO.cleanup()
    return outstr
